...
2. **Quyết định:** Kiểm tra citation chặt: mọi `[...]` trong câu trả lời phải là ID có trong `sources`, nếu không thì trả safe refusal.
   **Lý do/evidence:** Contract yêu cầu "citation phải đối chiếu được với phần tử trong `sources`"; cách này chặn citation bịa ID (`b372db2`). Evaluation: 3/3 câu ngoài domain được từ chối ở cả hai config.
   **Trade-off:** Có từ chối sai. Theo `RESULT.md`, câu #5 (bán vé lễ hội, config B) có evidence đầy đủ (recall 1.0) nhưng vẫn ra safe refusal; lần chạy 1 cũng có #1 và #15.
...
- Lỗi đã phát hiện và cách xử lý: tokenizer BM25 ban đầu tách bằng `split()` nên dấu câu dính vào từ ("Gióng?" không khớp "Gióng,"); nhóm đổi sang tách theo `\w+` và chuẩn hoá NFC (`4bace13`).

## Điều còn hạn chế

- Một hạn chế cụ thể của phần tôi làm: `SYSTEM_PROMPT` ngắn và nhấn mạnh việc từ chối, cộng với kiểm tra citation chặt, khiến model nhỏ đôi khi từ chối dù context đủ thông tin; kết quả không ổn định giữa các lần chạy (temperature 0.3).
